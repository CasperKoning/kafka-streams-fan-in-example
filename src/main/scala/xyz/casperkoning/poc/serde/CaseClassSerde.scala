...
class CaseClassSerde[CC >: Null](schemaRegistryUrl: String = "http://schema_registry:8081", isKey: Boolean)(implicit format: RecordFormat[CC]) extends Serde[CC] {
  private class CaseClassDeserializer(schemaRegistryUrl: String = "http://schema_registry:8081", isKey: Boolean)(implicit format: RecordFormat[CC]) extends Deserializer[CC] {
    ...
    override def deserialize(topic: String, data: Array[Byte]): CC = if(data != null) {
  ...
  private class CaseClassSerializer(schemaRegistryUrl: String = "http://schema_registry:8081", isKey: Boolean)(implicit format: RecordFormat[CC]) extends Serializer[CC] {
    ...
    override def serialize(topic: String, data: CC): Array[Byte] = if (data != null) {
